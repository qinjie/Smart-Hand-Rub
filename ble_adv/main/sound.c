...
void play_low_sound() {
	sound(GPIO_OUTPUT,c,200);
	//vTaskDelay(150/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,e,200);
	//vTaskDelay(300/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,g,200);
	//vTaskDelay(300/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,aS,400);
	//vTaskDelay(100/portTICK_PERIOD_MS);
}

void play_heigh_sound() {
	sound(GPIO_OUTPUT,fH,200);
	//vTaskDelay(150/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,fH,100);
	//vTaskDelay(300/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,fH,100);
	//vTaskDelay(300/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,gH,300);
	sound(GPIO_OUTPUT,aH,400);
	//vTaskDelay(100/portTICK_PERIOD_MS);
}
...
void fun() {
	...
	sound(GPIO_OUTPUT,660,200);
	//vTaskDelay(100/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,660,200);
	//vTaskDelay(100/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,510,60);
	//vTaskDelay(30/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,660,200);
	//vTaskDelay(100/portTICK_PERIOD_MS);
	sound(GPIO_OUTPUT,770,400);
}
...
void gpio_task(void *pvParameters) {
	...
	while (1) {
		bits=xEventGroupWaitBits(alarm_eventgroup, GPIO_SENSE_BIT,pdTRUE, pdFALSE, 1000 / portTICK_RATE_MS); // max wait 60s
		if(bits!=0) {
			if (loop%2==0) {
				play_march(0);
			} else {
